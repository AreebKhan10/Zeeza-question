...
export default function SingleQuestion({
  ...
  useEffect(() => {
    ...
    const config = {
      headers: {
        Authorization: "bksadbfoabd",
      },
    };
    const url =
      "https://31zctjiomj.execute-api.us-east-1.amazonaws.com/default/enhacereport?StudentID=2";
    ...
    if (ID == 5 && QuesID == 0) {
      ...
      if (isChecked) {
        ...
        if(currentValue != 'none'){
          if (allChecked.length < Data[ID].questions[QuesID].max) {
            var noneIndex = Data[ID].questions[QuesID].options.findIndex(x => x.value == 'none');
  ...
  return (
    <div className="my-14 ml-40 mb-32">
      ...
      <p className="text-xl text-[#002B48]">{value[QuesID]?.toString()}</p>
      <p className="p-2 text-blue-800">
